...
def fork_session(
    ...
    if up_to_message_id:
        ...
        for i, entry in enumerate(transcript):
            if entry.get("uuid") == up_to_message_id:
                ...
                break
        if cutoff == -1:
            raise ValueError(f"Message {up_to_message_id} not found in session {session_id}")
    ...
    for i, original in enumerate(writable):
        ...
        new_logical_parent = uuid_mapping.get(logical_parent) if logical_parent else logical_parent
    ...
    if not fork_title:
        buf_len = len(content)
        head = content[: min(buf_len, LITE_READ_BUF_SIZE)].decode("utf-8", errors="replace")
        tail = content[max(0, buf_len - LITE_READ_BUF_SIZE) :].decode("utf-8", errors="replace")
        base = (
...
def _append_to_session(session_id: str, data: str, directory: str | None) -> None:
    ...
    try:
        ...
        raise FileNotFoundError(f"Session {session_id} not found (no projects directory)") from e
    for entry in dirents:
...
def _sanitize_unicode(value: str) -> str:
    ...
    for _ in range(10):
        ...
        current = "".join(c for c in current if unicodedata.category(c) not in _FORMAT_CATEGORIES)
