...
from clawd_code_sdk.models.session import SDKSessionInfo, SessionMessage
                    ...
                    case list():
                        for block in content:
                            if (
                                ...
                                texts.append(block["text"])  # noqa: PERF401
            case _:
                continue
        for raw in texts:
            result = raw.replace("\n", " ").strip()
            ...
            if len(result) > 200:  # noqa: PLR2004
...
def _parse_session_info_from_lite(
    ...
    tag = (_extract_last_json_string_field(tag_line, "tag") or None) if tag_line else None
...
def _build_conversation_chain(
    ...
    main_leaves = [
        leaf
        for leaf in leaves
        if not leaf.get("isSidechain") and not leaf.get("teamName") and not leaf.get("isMeta")
    ]
