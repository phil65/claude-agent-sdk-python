...
from clawd_code_sdk._internal.session_mutations import _sanitize_unicode, _try_append
from clawd_code_sdk._internal.sessions import _sanitize_path, get_session_messages
...
def _make_session_file(
    ...
    lines = [
        json.dumps({"type": "user", "message": {"role": "user", "content": first_prompt}}),
...
class TestRenameSession:
    ...
    def test_no_projects_dir_raises(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    def test_title_trimmed_before_storing(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_compact_json_format(self, claude_config_dir: Path, tmp_path: Path):
        ...
        assert lines[-1] == (f'{{"type":"custom-title","customTitle":"Title","sessionId":"{sid}"}}')
...
class TestTagSession:
    ...
    def test_last_wins(self, claude_config_dir: Path, tmp_path: Path):
        ...
        tag_lines = [json.loads(line) for line in lines if json.loads(line).get("type") == "tag"]
    ...
    def test_sanitization_rejects_pure_invisible(self, claude_config_dir: Path, tmp_path: Path):
...
class TestDeleteSession:
    ...
    def test_removes_subagent_transcript_dir(self, claude_config_dir: Path, tmp_path: Path):
