...
from clawd_code_sdk._internal.sessions import (
...
class TestListSessions:
    ...
    def test_summary_wins_first_prompt(self, claude_config_dir: Path, tmp_path: Path):
        ...
        Path(project_path).mkdir(parents=True)
        project_dir = _make_project_dir(claude_config_dir, os.path.realpath(project_path))
        _make_session_file(project_dir, first_prompt="question", summary="better summary")
    ...
    def test_multiple_sessions_sorted_by_mtime(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_limit(self, claude_config_dir: Path, tmp_path: Path):
        ...
        for i in range(5):
            _make_session_file(project_dir, first_prompt=f"prompt {i}", mtime=1000.0 + i)

        sessions = list_sessions(directory=project_path, limit=2, include_worktrees=False)
    ...
    def test_offset_pagination(self, claude_config_dir: Path, tmp_path: Path):
        ...
        # Get page 1 (first 2)
        page1 = list_sessions(directory=project_path, limit=2, offset=0, include_worktrees=False)
        assert len(page1) == 2

        # Get page 2 (next 2)
        page2 = list_sessions(directory=project_path, limit=2, offset=2, include_worktrees=False)
        assert len(page2) == 2
        ...
        page_empty = list_sessions(directory=project_path, offset=100, include_worktrees=False)
    ...
    def test_list_all_sessions_dedupes(self, claude_config_dir: Path):
        ...
        _make_session_file(proj1, session_id=shared_sid, first_prompt="older", mtime=1000.0)
        _make_session_file(proj2, session_id=shared_sid, first_prompt="newer", mtime=2000.0)
    ...
    def test_limit_zero_returns_all(self, claude_config_dir: Path, tmp_path: Path):
        ...
        sessions = list_sessions(directory=project_path, limit=0, include_worktrees=False)
        assert len(sessions) == 3

    def test_cwd_from_head_fallback_to_project_path(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_git_branch_from_tail_preferred(self, claude_config_dir: Path, tmp_path: Path):
...
class TestGetSessionMessages:
    ...
    def test_filters_non_user_assistant_from_chain(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_limit_and_offset(self, claude_config_dir: Path, tmp_path: Path):
        ...
        for i, uid in enumerate(uuids):
            ...
            entries.append(_make_transcript_entry(entry_type, uid, parent, sid, content=f"m{i}"))
        _write_transcript(project_dir, sid, entries)
    ...
    def test_picks_main_chain_over_sidechain(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_picks_latest_leaf_by_file_position(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_terminal_non_message_walked_back(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_corrupt_lines_skipped(self, claude_config_dir: Path, tmp_path: Path):
        ...
        lines = [
            ...
            "not valid json {{{",
            "",
            json.dumps(_make_transcript_entry("assistant", a1, u1, sid, content="hello")),
        ]
        (project_dir / f"{sid}.jsonl").write_text("\n".join(lines) + "\n")
    ...
    def test_ignores_non_transcript_types(self, claude_config_dir: Path, tmp_path: Path):
...
class TestTagExtraction:
    ...
    def test_tag_last_wins(self, claude_config_dir: Path, tmp_path: Path):
        ...
        lines = [
            json.dumps({"type": "user", "message": {"content": "hello"}}),
            json.dumps({"type": "tag", "tag": "first-tag", "sessionId": sid}, **_COMPACT),
            json.dumps({"type": "tag", "tag": "second-tag", "sessionId": sid}, **_COMPACT),
        ]
        file_path.write_text("\n".join(lines) + "\n")
    ...
    def test_tag_ignores_tool_use_inputs(self, claude_config_dir: Path, tmp_path: Path):
        ...
        lines = [
            ...
            json.dumps({"type": "tag", "tag": "real-tag", "sessionId": sid}, **_COMPACT),
    ...
    def test_tag_none_when_only_tool_use_tag(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_parse_session_info_from_lite_helper(self, tmp_path: Path):
        ...
        lines = [
            ...
            json.dumps({"type": "tag", "tag": "experiment", "sessionId": sid}, **_COMPACT),
        ]
        file_path.write_text("\n".join(lines) + "\n")
...
class TestCreatedAtExtraction:
    ...
    def test_created_at_from_iso_timestamp(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_created_at_leq_last_modified(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_created_at_none_when_missing(self, claude_config_dir: Path, tmp_path: Path):
...
class TestGetSessionInfo:
    ...
    def test_found_with_directory(self, claude_config_dir: Path, tmp_path: Path):
        ...
        Path(project_path).mkdir(parents=True)
        project_dir = _make_project_dir(claude_config_dir, os.path.realpath(project_path))
        sid, _ = _make_session_file(project_dir, first_prompt="hello", git_branch="main")
    ...
    def test_returns_none_for_sidechain(self, claude_config_dir: Path, tmp_path: Path):
        ...
        Path(project_path).mkdir(parents=True)
        project_dir = _make_project_dir(claude_config_dir, os.path.realpath(project_path))
        sid, _ = _make_session_file(project_dir, first_prompt="sidechain", is_sidechain=True)

        assert get_session_info(sid, directory=project_path) is None

    def test_directory_not_containing_session(self, claude_config_dir: Path, tmp_path: Path):
...
class TestListSubagents:
    ...
    def test_session_exists_no_subagents_dir(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_empty_subagents_dir(self, claude_config_dir: Path, tmp_path: Path):
        ...
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        sid, _ = _make_session_with_subagents(claude_config_dir, project_path, agent_ids=[])
    ...
    def test_recurses_into_subdirectories(self, claude_config_dir: Path, tmp_path: Path):
...
class TestGetSubagentMessages:
    ...
    def test_nonexistent_agent(self, claude_config_dir: Path, tmp_path: Path):
        ...
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        sid, _ = _make_session_with_subagents(claude_config_dir, project_path, agent_ids=["other"])
    ...
    def test_simple_chain(self, claude_config_dir: Path, tmp_path: Path):
        ...
        project_path = str(tmp_path / "proj")
        Path(project_path).mkdir(parents=True)
        sid, subagents_dir = _make_session_with_subagents(claude_config_dir, project_path)
    ...
    def test_finds_agent_in_nested_subdirectory(self, claude_config_dir: Path, tmp_path: Path):
    ...
    def test_limit_and_offset(self, claude_config_dir: Path, tmp_path: Path):
        ...
        page = get_subagent_messages(sid, "p", directory=project_path, limit=2, offset=2)
